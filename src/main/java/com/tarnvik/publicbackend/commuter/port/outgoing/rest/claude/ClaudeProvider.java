...
import com.tarnvik.publicbackend.commuter.port.outgoing.rest.claude.dto.DeviationResponse;
...
public class ClaudeProvider {
  ...
  public ClaudeProvider(@Value("${anthropic.apiKey}") String apiKey) {
  ...
  public DeviationResponse interpretDeviation(String deviation) {
    ...
    StructuredMessageCreateParams<DeviationResponse> params = MessageCreateParams.builder()
      .model(Model.CLAUDE_HAIKU_4_5)
      .maxTokens(512)
      .addUserMessage(prompt)
      .outputConfig(DeviationResponse.class)
      .build();

    StructuredMessage<DeviationResponse> response = client.messages().create(params);
    DeviationResponse deviationResponse = response.content().get(0).asText().text();
