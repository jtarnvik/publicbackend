...
| GET | `/api/protected/gtfs/resolve-trip` | User | Finds the live vehicle behind one SL departure row, so a click on it can open the schematic with that vehicle selected. Params are a content key, not an id: `transportMode`, `routeGroup`, `line`, `stopAreaId`, `stopAreaName`, `scheduled`, `destination`. Always 200 — `outcome` is `MATCHED`, `NO_MATCH`, `AMBIGUOUS` or `NO_LIVE_DATA`, and only the first carries `tripId`/`vehicleId` |
...
| PUT | `/api/protected/settings/favourite-stops` | User | Save the favourite stops alone, from a tap on a stop in the live traffic schematic. `favouriteStops` is `@NotNull` here — unlike on `/settings`, every caller of this endpoint is new. Separate from `/settings` because the live view does not own the stop point and must not echo it back |
